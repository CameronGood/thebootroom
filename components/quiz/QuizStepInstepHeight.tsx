...
const getImagePath = (volume: string): string => {
  const basePath = "/quiz/";
  // Match the actual filenames: "Instep Low.svg", "Instep Average.svg", "Instep High.svg"
  return `${basePath}Instep ${volume}.svg`;
};
...
function QuizStepInstepHeight({
  ...
  return (
    <QuizStepLayout
      title="Instep"
      description="Select the image that best matches your instep."
      ...
      <div className="flex flex-col items-center space-y-3 lg:space-y-4 max-w-2xl mx-auto">
        ...
        <div className="w-full max-w-md md:-mt-[40px]">
          <style dangerouslySetInnerHTML={{__html: `
            .instep-slider::-webkit-slider-thumb {
              appearance: none;
              width: 20px;
              height: 20px;
              background: #F5E4D0;
          ...
          <input
            ...
            className="instep-slider w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#F5E4D0]"
            style={{
